...
import com.asledz.kancelaria_prawnicza.utilis.QueriesGenerator;
import com.asledz.kancelaria_prawnicza.utilis.SearchUtils;
import com.asledz.kancelaria_prawnicza.utilis.SortGenerator;
...
public class DocumentService {
    ...
    public Page<DocumentDTO> getDocuments(Map<String, String> pageProperties) {
        int page = Integer.parseInt(pageProperties.get("page")) - 1;
        pageProperties.remove("page");
        log.info("Page %d of all documents".formatted(page));
        int pageSize = Integer.parseInt(pageProperties.get("pageSize"));
        pageProperties.remove("pageSize");
        Pageable paging;
    ...
    public Page<DocumentDTO> getDocumentsByTypeAndText(Integer page, String prompt) {
        log.info("Page %d of all documents".formatted(page));
        int pageSize = 5;
        page -= 1;
        Pageable paging = PageRequest.of(page, pageSize, Sort.by("id"));
    ...
    public Page<DocumentDTO> getDocumentsByParameters(MultiValueMap<String, String> parameters) {
        ...
        if (parameters.containsKey("page")) {
            page = Integer.parseInt(parameters.get("page").get(0)) - 1;
            parameters.remove("page");
        }
        log.info("Page %d of all documents".formatted(page));
        int pageSize = 5;
        if (parameters.containsKey("pageSize")) {
            pageSize = Integer.parseInt(parameters.get("pageSize").get(0));
